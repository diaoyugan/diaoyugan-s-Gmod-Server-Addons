...
CrosshairDesigner.OpenMenu = function(resolutionChanged)
    ...
    toClipboard:SetText("复制当前准星到剪贴板")
	...
	local toClipboard = vgui.Create("DButton", CrosshairDesigner.Sheet.Saving)
    toClipboard:SetText("从字符串加载准星 (CTRL+V 粘贴)")
    toClipboard:Dock(TOP)
	toClipboard:DockMargin(0, 5, 0, 0)
    toClipboard.DoClick = function()
    	Derma_StringRequest(
    ...
    resetCrosshair:SetText("重置准星为默认值")
    ...
    label:SetWrap(true)
    label:SetText("缓存用于在屏幕上每个位置生成准星的计算 缓存越大，使用的内存越多，但需要重新计算的次数越少，缓存使用越高 - Can be left at 1 for static crosshairs (dynamic off + centre to player angles off)")
    label:SetDark(1)
    label:Dock(TOP)
	label:DockMargin(0, 5, 0, 0)
	...
	label.Recalculate = function()
		...
		label:SetText("当前估计内存使用量: " .. CrosshairDesigner.FormatBytes(mem))
	end
	label.Recalculate()
